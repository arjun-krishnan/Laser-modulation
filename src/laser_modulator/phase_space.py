...
def define_bunch(
    ...
    CS_inv_x = np.abs(
        np.random.normal(loc=0, scale=emitX * np.sqrt(2 * np.pi), size=N_e)
    )
    CS_inv_y = np.abs(
        np.random.normal(loc=0, scale=emitY * np.sqrt(2 * np.pi), size=N_e)
    )
    ...
    elec0[1] = (
        -np.sqrt(CS_inv_x / betaX) * (alphaX * np.cos(phase_x) + np.sin(phase_x))
        + elec0[5, :] * Dxprime
    )
...
def coord_change(elec_dummy, e_E):
    ...
    elec[5, :] = p_elecs / (
        np.sqrt(1 / np.cos(elec_dummy[1, :]) ** 2 + np.tan(elec_dummy[3, :]) ** 2)
    )
...
def calc_bn(tau0, wl, printmax=True):
    ...
    if printmax:
        print(
            "Maximum bunching factor is",
            np.round(max(bn), 4),
            " at ",
            np.round(wl_max * 1e9, 2),
            " nm",
        )
    return np.array(bn)
