...
class Laser:
    def __init__(self,wl,sigx,pulse_len,pulse_E,focus,M2=1.0,pulsed=True,phi=0):
...
def define_B_field(Bmax,und_periodlen,und_n_periods):
    und_len=und_periodlen*und_n_periods
    s=np.linspace(0,und_len,1000)
    B=Bmax*np.sin(2*np.pi*s/und_periodlen)
    i=0
    while(s[i]<und_periodlen):
        if(s[i]<und_periodlen/2):
            B[i]*=0.25
        else:
            B[i]*=0.75
        i+=1
    ...
    i=-1
    while(-s[i]+und_len<und_periodlen):
        if(-s[i]+und_len<und_periodlen/2):
            B[i]*=0.25
        else:
            B[i]*=0.75
        i-=1
        
    B_func=interpolate.interp1d(s,B) 
    return(B_func)
